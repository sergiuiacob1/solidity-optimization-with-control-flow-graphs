...
contract PQEthBridge is Ownable, Pausable {

    IERC20 public token;

    address public manager;

    uint256 public commission = 10000000000000000; // 0.004 eth starting commission
    uint256 public maxTransfer = 0; // max token amount the bridge will take in one tx

    address payable public marketingWallet = 0x36E58a20C7b96B3503Dc9F2c6aba3af1f4876c83;
    ...
    modifier restricted {
        ...
        require(isOwner == true, '!restricted');
    ...
    function sendTokens(uint256 amount) public payable notPaused {
        ...
        if(maxTransfer > 0) {
            require(amount <= maxTransfer, "trying to transfer too many tokens");
        }
    ...
    function writeTransaction(address user, uint256 amount) public payable restricted {
    ...
    function recieveTokens() public payable notPaused {

        if (tokensRecievedButNotSent[msg.sender] != 0) {

            marketingWallet.transfer(commission);
                    
            uint256 amountToSent;
             
            amountToSent = tokensRecievedButNotSent[msg.sender] - tokensSent[msg.sender];

            require(token.balanceOf(address(this)) >= amountToSent, "Not enough tokens in contract");
    ...
    function withdrawTokens(uint256 amount, address reciever) public onlyOwner {
    ...
    function withdrawEther(uint256 amount, address payable reciever) public onlyOwner {
    ...
    function setMaxTransfer(uint256 _amount) public onlyOwner {
