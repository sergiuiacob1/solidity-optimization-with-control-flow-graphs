...
contract Depth is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ; uint256 totalSupply = 1 * 10 ** 8 * 10 ** decimals();
        ...
        maxTransactionAmount = 1 * 10 ** 6 * 10 ** decimals(); // 1% maxTransactionAmountTxn
        maxWallet = 2 * 10 ** 6 * 10 ** decimals(); // 2% maxWallet
