...
contract Rexalla is EIP712, Ownable, ERC721A {
    ...
    function mintPresaleNFT(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= maxPerTransaction, "Wrong Quantity.");
        require(totalSupply() + _quantity <= maxSupply, "Reaching max supply");
        require(totalSupply() + _quantity <= Presale_Supply, "Reaching Presale Supply");
        require(msg.value == presale_price * _quantity, "Needs to send more eth");
        require(getMintedCount(msg.sender) + _quantity <= maxInAddress, "Exceed max minting amount");
        require(presaleSaleState, "Presale Not Started Yet!");

        _safeMint(msg.sender, _quantity);

    }

    function mintWL(uint256 _quantity, uint256 id, bytes memory signature) public payable {
        require(_quantity > 0 && _quantity <= maxPerTransaction, "Wrong Quantity.");
        require(totalSupply() + _quantity <= WL_Supply, "Max WL supply reached.");
        require(totalSupply() + _quantity <= maxSupply, "Reaching max supply");
        require(msg.value == WL_price * _quantity, "Needs to send more eth");
        require(getMintedCount(msg.sender) + _quantity <= maxInAddressWL, "Exceed max minting amount");
   ...
   function sendGiftsToWallet(address _wallet, uint256 _num) external onlyOwner{
    ...
    function updateWL_Supply(uint256 _supply) public onlyOwner() {
        WL_Supply = _supply;
    }
    function updatePrice(uint256 _price) public onlyOwner() {
        price = _price;
    }
    function updateWLPrice(uint256 _price) public onlyOwner() {
        WL_price = _price;
    }
    function updatePresalePrice(uint256 _price) public onlyOwner() {
    ...
    function setmaxInAddresss(uint256 _maxInAddress,  uint256 _maxInAddressWL) public onlyOwner() {
        maxInAddress = _maxInAddress;
        maxInAddressWL = _maxInAddressWL;
    }
    ...
    function togglePresaleState() public onlyOwner() {
        presaleSaleState = !presaleSaleState;
    }
    function togglewlState() public onlyOwner() {
        wlState = !wlState;
    }
