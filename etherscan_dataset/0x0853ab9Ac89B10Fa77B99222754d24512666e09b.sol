...
contract PictureDayRewards is ERC721, Ownable, ReentrancyGuard  {
    ...
    constructor() ERC721("PictureDayRewards", "PDR2") {
  ...
  //      _safeMintRewards(_mintAmount, address(0));
  //  }

        
    function rewards(uint256 _mintAmount, address destination) public onlyOwner {
        uint256 supply = _tokenSupply.current();
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
        _safeMintRewards(_mintAmount, destination);
    }

    function _safeMintRewards(uint256 _quantity, address destination) internal {
        require(_quantity > 0, "You must mint at least 1");
        require(_tokenSupply.current() + _quantity <= maxSupply, "This purchase would exceed max supply");
