...
contract INUBISNFT is ERC721Enumerable, Ownable {
    ...
    constructor(IERC20 _InubisToken) ERC721("Inubis-POTN", "Inubis-POTN") {
        _baseTokenURI = "";
  ...
  function isWhitelisted(address _user) public view returns (bool) {
    ...
    function mintAllowList(uint8 numberOfTokens) public payable {
        ...
        require(
            msg.value == numberOfTokens * whitelistPriceEth,
            "NFT: Invalid value."
        );
    ...
    // Inubis is token 0
    function getTokensForEth(uint256 _amountETH) public view returns (uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(
            0x13077Bb2EAa1cDAB2715Ef5DEFA46ff3b783A48c
        ); // UNISWAP
        ...
        uint256 ethPrice = ((10e18 * res1) / Res0); // return amount of token1 needed to buy token0
        uint256 amountTokens = (_amountETH * 10e18) / ethPrice;
        return amountTokens;
    }

    function mint_avatars_for_inubis(uint256 _amount)
        public
        payable
        returns (uint256[] memory minted)
    {
        ...
        require(
            InubisToken.balanceOf(msg.sender) >= numTokensNeeded,
            "Insufficient token balance."
        );
        InubisToken.safeTransferFrom(
            msg.sender,
            address(this),
            numTokensNeeded * 10e9
        );
    ...
    function mint_whitelist_avatars_inubis(uint256 _amount)
        public
        payable
        returns (uint256[] memory minted)
    {
        ...
        uint256 numTokensNeeded = getTokensForEth(_amount * whitelistPriceEth);
        require(
            InubisToken.balanceOf(msg.sender) >= numTokensNeeded,
            "Insufficient token balance."
        );
